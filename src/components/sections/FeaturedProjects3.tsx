    ...
    >
      ...
      <div className="mt-4 flex justify-between items-center">
        <div className="flex flex-col">
          <h3 className="text-lg md:text-xl font-sans font-bold tracking-tight uppercase text-white">
            {project.name}
          </h3>
          <AnimatePresence>
            {isHovered && (
              <motion.p 
                ...
                className="hidden md:block text-[9px] uppercase tracking-[0.2em] text-white/40 font-sans overflow-hidden"
              >
                {project.tags.join(" • ")}
              </motion.p>
            )}
        ...
        <span className="text-[11px] font-sans font-bold text-white/30 tracking-widest">
          2024
        </span>
      </div>
    </motion.div>
...
export const FeaturedProjects3 = () => {
  ...
  useEffect(() => {
    ...
    // Cleanup to ensure sidebar is visible if section is unmounted
    ...
    >
      <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-[10px] items-start">
        {projects.map((project, index) => {
          // Repeating pattern of 4 projects:
          // 0: Big (Left)
          // 1: Small (Right)
          // 2: Small (Left)
          // 3: Big (Right)
          ...
          let colSpan = "md:col-span-5"; // Default Small
          let heightClass = "h-[40vh] md:h-[50vh]"; // Default Small height

          if (position === 0 || position === 3) {
            colSpan = "md:col-span-7"; // Big
            heightClass = "h-[60vh] md:h-[80vh]"; // Big height
          }
